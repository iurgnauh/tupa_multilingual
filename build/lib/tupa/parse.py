...
class PassageParser(AbstractParser):
    ...
    def init(self):
        ...
        for model in self.models:
            model.init_model(self.config.format, lang=self.lang)
            #if model.feature_extractor and model.classifier:
            #    model.save(suffix='.after_init')
...
class Parser(AbstractParser):
    ...
    def __init__(self, model_files=(), config=None, beam=1):
        ...
        self.best_dev_score = self.best_test_score = self.dev = self.test = self.iteration = self.epoch = self.batch = None
        self.trained = self.save_init = False
        self.best_epoch_dev = self.best_epoch_test = -1
                ...
                if dev:
                    if self.iteration < len(iterations):
                        if self.model.is_retrainable:
                            self.model.load(is_finalized=False, suffix=".test.{}".format(self.best_epoch_test))  # Load best model to prepare for next iteration
                    elif test:
                        self.model.load(suffix=".test.{}".format(self.best_epoch_test))  # Load best model to prepare for test
    ...
    def init_train(self):
        ...
        self.print_config()
        self.best_dev_score = self.model.classifier.best_score if self.model.classifier else 0
    ...
    def eval_and_save(self, last=False, finished_epoch=False):
        ...
        #self.save(self.model, suffix=".before.{}".format(self.epoch))
        self.model = finalized = model.finalize(finished_epoch=finished_epoch)
        if self.dev:
            if not self.best_dev_score:
                #self.save(finalized)
                self.save(finalized, suffix=".dev.{}".format(self.epoch))
            average_score, scores = self.eval(self.dev, ParseMode.dev, self.config.args.devscores)
            if average_score >= self.best_dev_score:
                print("Better than previous best dev score (%.3f)" % self.best_dev_score)
                finalized.classifier.best_score = average_score
                if self.best_dev_score:
                    self.save(finalized, suffix=".dev.{}".format(self.epoch))
                    #self.save(finalized)
                self.best_dev_score = average_score
                self.best_epoch_dev = self.epoch
                ...
                print("Not better than previous best dev score (%.3f)" % self.best_dev_score)
        elif last or self.config.args.save_every is not None:
            self.save(finalized, suffix=".last.{}".format(self.epoch))

        if self.test:
            if not self.best_test_score:
                #self.save(finalized)
                self.save(finalized, suffix=".test.{}".format(self.epoch))
            average_score, scores = self.eval(self.test, ParseMode.dev, self.config.args.testscores)
            if average_score >= self.best_test_score:
                print("Better than previous best test score (%.3f)" % self.best_test_score)
                # finalized.classifier.best_score = average_score
                if self.best_test_score:
                    self.save(finalized, suffix=".test.{}".format(self.epoch))
                    #self.save(finalized)
                self.best_test_score = average_score
                self.best_epoch_test = self.epoch
                #if self.test and self.test is not True:  # There are actual passages to parse
                #    self.eval(self.test, ParseMode.test, self.config.args.testscores, display=False)
            else:
                print("Not better than previous best test score (%.3f)" % self.best_test_score)

        if not last:
            finalized.restore(model)  # Restore non-finalized model
            #self.save(finalized, suffix=".restore.{}".format(self.epoch))
        return scores

    def save(self, model, suffix=None):
        self.config.save(model.filename)
        model.save(save_init=self.save_init, suffix=suffix)
...
def read_passages(args, files, lang=None):
    ...
    if args.folds:
        ...
        if 'train' in files:
            for train_file in files['train']:
                lang = train_file[0]
                filename = train_file[1]
                tmp_passages = read_passages(args, [filename], lang=lang)
                for p in tmp_passages:
                    p.attrib['lang'] = lang
                    train_passages.append(p)
                print('Loaded train file {}, language {}'.format(filename, lang))
        
        if 'dev' in files:
            for dev_file in files['dev']:
                lang = dev_file[0]
                filename = dev_file[1]
                tmp_passages = read_passages(args, [filename], lang=lang)
                for p in tmp_passages:
                    p.attrib['lang'] = lang
                    dev_passages.append(p)
                print('Loaded dev file {}, language {}'.format(filename, lang))
        
        if 'test' in files:
            for test_file in files['test']:
                lang = test_file[0]
                filename = test_file[1]
                tmp_passages = read_passages(args, [filename], lang=lang)
                for p in tmp_passages:
                    p.attrib['lang'] = lang
                    test_passages.append(p)
                print('Loaded test file {}, language {}'.format(filename, lang))
