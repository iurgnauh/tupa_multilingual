...
class ParameterDefinition:
    ...
    def create_from_config(self, lang=None):
        ...
        for k, v in self.attr_to_arg.items():
            if k == 'filename' and self.name == 'W':
                assert self.args.word_vectors
                import json
                with open(self.args.word_vectors) as f:
                    vector_files = json.load(f)
                print(vector_files)
                assert lang in vector_files
                kwargs.update({k: vector_files[lang]})
                """
                #print('in word vector if', lang)
                if lang == 'en':
                    kwargs.update({k: 'test_multilingual/glove.sample.en'})
                else:
                    kwargs.update({k: 'test_multilingual/glove.sample.fr'})
                """
            else:
                kwargs.update({k: getattr(self.get_args(lang), v)})
...
PARAM_DEFS = [
    ...
    ("w",            dict(dim="word_dim",       size="max_words",  dropout="word_dropout"),  dict(lang_specific=False)),
    ("m",            dict(dim="lemma_dim",      size="max_lemmas", dropout="lemma_dropout"), dict(lang_specific=False)),
    ("t",            dict(dim="tag_dim",        size="max_tags",   dropout="tag_dropout"),   dict(lang_specific=False)),
    ...
    ("#",            dict(dim="shape_dim",      size="max_shapes"),                          dict(lang_specific=False)),
    ("^",            dict(dim="prefix_dim",     size="max_prefixes"),                        dict(lang_specific=False)),
    ("$",            dict(dim="suffix_dim",     size="max_suffixes"),                        dict(lang_specific=False)),
]
...
class Model:
    ...
    def init_model(self, axis=None, lang=None, init_params=True):
        #lang = 'fr'
        #print('arg_lang: {}, self_lang: {}'.format(lang, self.lang))
        ...
        if init_params:  # Actually use the config state to initialize the features and hyperparameters, otherwise empty
            for param_def in self.param_defs():  # FIXME save parameters separately per format, not just per language
                assert self.config.args.all_langs
                #print(self.config.args.all_langs)
                #input()
                for param_lang in self.config.args.all_langs \
                        if param_def.lang_specific else [None]:
                    ...
                    if param:
                        param.enabled = enabled and (not param_lang or param_lang == self.lang)
                    elif self.is_neural_network and enabled:
                        self.feature_params[key] = param_def.create_from_config(param_lang)
                        self.init_param(key)
                        self.feature_params[key].enabled = not param_lang or param_lang == self.lang
        ...
        #for key, param in self.feature_params.items():
        #    print(key)
        #exit(0)
        ...
        #print('end of init model')
        #self.save(suffix=".after_init")
    ...
    def save(self, save_init=False, suffix=None):
        """
        ...
        if self.filename is not None:
            ...
            try:
                self.feature_extractor.save(filename, save_init=save_init)
                ...
                self.classifier.save(filename, skip_labels=skip_labels,
                                     multilingual=self.config.args.multilingual,
                                     omit_features=self.config.args.omit_features)
                textutil.models["vocab"] = self.config.args.vocab
                save_json(filename + ".nlp.json", textutil.models)
                remove_backup(filename)
            except Exception as e:
                raise IOError("Failed saving model to '%s'" % filename) from e

    def load(self, is_finalized=True, suffix=None):
        """
        ...
        if self.filename is not None:
            ...
            if suffix:
                filename = filename + suffix
            try:
                self.config.args.classifier = Classifier.get_property(filename, "type")
                self.config.args.multilingual = Classifier.get_property(filename, "multilingual")
                self.config.args.omit_features = Classifier.get_property(filename, "omit_features")
                self.init_model(init_params=False)
                self.feature_extractor.load(filename, order=[p.name for p in self.param_defs()])
                if not is_finalized:
                ...
                self.load_labels()
                try:
                    textutil.models.update(load_json(filename + ".nlp.json"))
                ...
                raise IOError("Failed loading model from '%s'" % filename) from e
